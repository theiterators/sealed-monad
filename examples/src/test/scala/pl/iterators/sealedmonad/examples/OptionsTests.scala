...
trait OptionsTests[M[_]] extends Laws {
  ...
  def loginEqv(email: String,
    ...
    Example1
      .login[M](email, findUser, findAuthMethod, issueTokenFor, checkAuthMethodAction, authMethodFromUserIdF, mergeAccountsAction) <-> Example1
  ...
  def confirmEmailEqv(token: String,
  ...
  def options(implicit ArbAuth: Arbitrary[AuthMethod],
...
trait OptionsTestInstances {
  ...
  implicit val ArbitraryAuthMethod: Arbitrary[AuthMethod] = Arbitrary(
    for {
  ...
  implicit val ArbitraryLoginResponse: Arbitrary[LoginResponse] = Arbitrary(
    Gen.oneOf(
      Gen.oneOf(LoginResponse.Deleted,
